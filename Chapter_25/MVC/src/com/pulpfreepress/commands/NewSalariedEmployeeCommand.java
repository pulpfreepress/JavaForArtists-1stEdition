...
public class NewSalariedEmployeeCommand extends BaseCommand {
  public void execute(){
    if(its_model != null){
       String[] emp_info = its_view.getNewSalariedEmployeeInfo();
       try{
         its_model.createSalariedEmployee(emp_info[0], emp_info[1], emp_info[2],
                                      Integer.parseInt(emp_info[4]), Integer.parseInt(emp_info[5]),
                                      Integer.parseInt(emp_info[6]), emp_info[3], emp_info[7],
                                      Double.parseDouble(emp_info[10]));
        }catch(Throwable t){
